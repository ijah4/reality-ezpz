...
async def get_users_ezpz():
  return (await asyncio.to_thread(run_command, command + '--list-users')).split('\n')[:-1]


async def get_config_ezpz(username):
  local_command = command + f"--show-user {shlex.quote(username)} | grep -E '://|^\\{{\"dns\"'"
  return (await asyncio.to_thread(run_command, local_command)).split('\n')[:-1]


async def delete_user_ezpz(username):
  await asyncio.to_thread(run_command, command + f'--delete-user {shlex.quote(username)}')


async def add_user_ezpz(username):
  await asyncio.to_thread(run_command, command + f'--add-user {shlex.quote(username)}')
...
async def users_list(update: Update, context: ContextTypes.DEFAULT_TYPE, text, callback):
  keyboard = []
  for user in await get_users_ezpz():
...
async def show_user(update: Update, context: ContextTypes.DEFAULT_TYPE, username):
  ...
  reply_markup = InlineKeyboardMarkup(keyboard)
  await context.bot.send_message(chat_id=update.effective_chat.id, text=f'Config for "{username}":', parse_mode='HTML')
  ...
  for config in config_list:
    ...
    await context.bot.send_photo(chat_id=update.effective_chat.id, photo=bio.getvalue(),
                                 caption=config_text, parse_mode='HTML', reply_markup=reply_markup)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if len(response) == 1:
    if response[0] == 'start':
      ...
      await context.bot.send_message(chat_id=update.effective_chat.id, text='Button pressed: {}'.format(response[0]))
  if len(response) > 1:
  ...
  if 'expected_input' in context.user_data:
    ...
    if expected_input == 'username':
      ...
      if not username_regex.match(username):
        await update.message.reply_text('Username can only contains A-Z, a-z and 0-9, try another username.')
        await add_user(update, context)
        return
